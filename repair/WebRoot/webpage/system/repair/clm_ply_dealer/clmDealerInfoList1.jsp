...
<div class="easyui-layout" fit="true">
  <div region="center" style="padding:0px;border:0px">
   <input type="text" name="plyDealerCode" id="plyDealerCode" value="${plyDealerCode }"  style="display: none" />
  <t:datagrid name="clmDealerInfoList" title="理赔车商信息" nowrap = "false" actionUrl="clmDealerInfoController.do?datagrid" 
  	checkbox="true"	queryMode="group" idField="id" fit="true">
   ...
   <t:dgCol title="资质类型" field="dealerType" dictionary="clm_type" hidden="true" query="true"  width="120"></t:dgCol>
  ...
  <t:dgToolBar title="关联理赔车商信息"  icon="icon-save" url="clmDealerInfoController.do?save" funname="saveClmPlyDealer"></t:dgToolBar>
  </t:datagrid>
  </div>
 </div>
<script>
...
function saveClmPlyDealer(title,url,gname) {
	 gridname=gname;
   var plyDealerCode=$("#plyDealerCode").val();
	 var clmDealerCodes = [];
   	...
   	$.dialog.confirm('你确定关联理赔承保车商信息吗?', function(r) {
		   if (r) {
				for ( var i = 0; i < rows.length; i++) {
					clmDealerCodes.push(rows[i].dealerCode);
				}
				$.ajax({
					url : url,
					type : 'get',
					data : {
						plyDealerCode : plyDealerCode,
						clmDealerCodes : clmDealerCodes.join(',')
					},
					cache : false,
