...
public class RepairServiceImpl implements IRepairService {
	...
	@Autowired
	private IGiveRepInfoSubmitService giveRepInfoSubmitService;
	...
	public String dispatchSubmit(String requsetXml) throws Exception {
		...
		try {
			...
			if (RepairUtils.isEmpty(requsetXml)) {
				throw new Exception(RepairConstants.CHECK_FAILED_CODE
						+ "@报文内容为空！");
			}

			head = repairUtils.getHeadInfo(requsetXml, null, "/PACKET/HEAD");
			repairMainInfo = repairUtils.getRpairMainInfo(requsetXml, null,
					"/PACKET/BODY/BASE_INFO");
			repairMainInfo = repairUtils.getRpairMainInfo(requsetXml,
					repairMainInfo, "/PACKET/BODY/VEHICLE_INFO");
			repairMainInfo = repairUtils.getRpairMainInfo(requsetXml,
					repairMainInfo, "/PACKET/BODY/CLAIM_INFO");
			...
			if (!"".equals(vlidateInfo)) {
				throw new Exception(RepairConstants.CHECK_FAILED_CODE + "@"
						+ vlidateInfo + "车架号为" + repairMainInfo.getFrmNo());
			}
			...
			if ("".equals(rules)) {// 符合送修規則才送修
				...
				RepairMainInfo rPCInfo = repairMainDao.getRCInfo(hql,
						new Object[] { frmNo, reportNo });
				if (rPCInfo == null || rPCInfo.getId() == null) {
					if (null != repairMainInfo.getRepairNo()
							&& !"".equals(repairMainInfo.getRepairNo())) {
						repairMainInfo.setRepairSerialNo(reportNo + frmNo);
						repairMainInfo
								.setRepairState(RepairConstants.REPAIR_STATUS_1);
						...
						repairMainInfo.setRepairSerialNo(reportNo + frmNo);
						repairMainInfo
								.setRepairState(RepairConstants.REPAIR_STATUS_3);
						repairMainInfo.setCrtTm(new Date());
						repairMainInfo.setUpdTm(new Date());
						repairMainDao.addRCinfo(repairMainInfo);
					}
				...
				if (rPCInfo == null || rPCInfo.getId() == null) {
					repairMainInfo.setRepairSerialNo(reportNo + frmNo);
					repairMainInfo
							.setRepairState(RepairConstants.REPAIR_STATUS_2);// /不送修状态；
					repairRulesService.setRepairRule(repairMainInfo,
							RepairConstants.REPAIRRULE_2);
					repairMainInfo.setCrtTm(new Date());
					repairMainInfo.setUpdTm(new Date());
					repairMainDao.addRCinfo(repairMainInfo);
				} else {
			...
			e.printStackTrace();
			responseXml = repairUtils.errorXML(
					RepairUtils.getExceptionMessage(e), requestType);
		} finally {
			try {
				// 保存交互报文
				if (responseXml != null & requsetXml != null) {
					dispatchSubmitService.savePacket(frmNo, reportNo,
							responseXml, requsetXml, "调度提交_S1");
				}
			} catch (Exception e) {
				e.printStackTrace();
			}
		...
		try {
			...
			for (RepairMainInfo rcInfo : mainList) {
				...
				if ("00".equals(vehicleType)) { // 标的车
					...
					if (assessCode == null || assessName == null
							|| assessTelphone == null) {
						throw new Exception(RepairConstants.CHECK_FAILED_CODE
								+ "@" + "车辆所属类型为标的车时,定损员信息不能为空！");
					}
				...
				// hql = " From RepairMainInfo  where  frmNo= ? and reportNo=?";
					...
					List<RepairMainInfo> list = repairMainDao.getRClist(hql,
							new Object[] { repairSerialNo });
					if (list.size() > 0) {
						for (RepairMainInfo mianInfo : list) {
							mianInfo.setAssessCode(rcInfo.getAssessCode());
							mianInfo.setAssessName(rcInfo.getAssessName());
							mianInfo.setAssessTelphone(rcInfo
									.getAssessTelphone());
							mianInfo.setRepairSerialNo(rcInfo
									.getRepairSerialNo());
					...
					if ("".equals(rules)) {// 符合送修規則才送修
						...
						if (surveyorName == null || "".equals(surveyorName)) {
							...
							List<RepairMainInfo> list = repairMainDao
									.getRClist(
											hql,
											new Object[] { rcInfo.getReportNo() });
							if (list.size() > 0) {
						...
						if (rPCInfo == null || rPCInfo.getId() == null) {
							...
							repairRulesService.setRepairRule(rcInfo,
									RepairConstants.REPAIRRULE_2);
							rcInfo.setCrtTm(new Date());
							rcInfo.setUpdTm(new Date());
							repairMainDao.addRCinfo(rcInfo);
			...
			// 保存交互报文
			if (responseXml != null & requsetXml != null) {
				surveySubmitService.savePacket(mainList, responseXml,
						requsetXml, "查勘提交_S2");
			}
		}
		return responseXml;
	...
	public String verifyLossSubmit(String requsetXml) throws Exception {
		...
		try {
			...
			Head head = repairUtils.getHeadInfo(requsetXml, null,
					"/PACKET/HEAD"); // 获取报文头部对象
			...
			for (DealearInfo deInfo : DeInfoList) {
				...
				if (dealearInfo == null || dealearInfo.getId() == null) {
					...
					RepairMainInfo rmian = repairMainDao.getRCInfo(repairHql,
							new Object[] { frmNo, reportNo });
					if (rmian.getId() != null) {
			...
			// 保存交互报文
			if (responseXml != null & requsetXml != null) {
				verifyLossSubmitService.savePacket(DeInfoList, responseXml,
						requsetXml, "核损通过告知_S3");
			}
		}
		return responseXml;
	}
	...
	/**
	 * 返回修理信息提交 param:requsetXml return: responeXml
	 * 
	 * @param requsetXml
	 */
	public String giveRepInfoSubmit(String requsetXml) throws Exception {
		RepairValidateService repairValidateService = RepairValidateService.getInstance();
		RepairUtils repairUtils = new RepairUtils();
		String responseXml = null;
		String requestType = null;
		Head head = null;
		RepairMainInfo repairMainInfo = null;
		List<MaintenanceRemarkEntity> rEmarkList = null;
		String frmNo = "";
		String reportNo = "";
		try {
			logger.info("收到修理信息提交请求报文:\n" + requsetXml);
			// 判断报文内容是否为空
			if (RepairUtils.isEmpty(requsetXml)) {
				throw new Exception(RepairConstants.CHECK_FAILED_CODE
						+ "@报文内容为空！");
			...
			frmNo = repairMainInfo.getFrmNo();
			reportNo = repairMainInfo.getReportNo();
			String policyNo = repairMainInfo.getPolicyNo();
			String existVerified = repairValidateService
					.existVerified(repairMainInfo);
			if (!"".equals(existVerified)) {
				throw new Exception(RepairConstants.CHECK_FAILED_CODE + "@"+ existVerified + "报案号为" + repairMainInfo.getReportNo());
			}
			responseXml = repairValidateService.resopneDangerInfoXml(
					repairMainInfo, rEmarkList);
		} catch (Exception e) {
			e.printStackTrace();
			responseXml = repairUtils.errorXML(
					RepairUtils.getExceptionMessage(e), requestType);
			...
			// 保存交互报文
			if (responseXml != null & requsetXml != null) {
				giveRepInfoSubmitService.savePacket(frmNo, reportNo,
						responseXml, requsetXml, "返回修理信息提交_S4");
			}
		}
		return responseXml;
	}
