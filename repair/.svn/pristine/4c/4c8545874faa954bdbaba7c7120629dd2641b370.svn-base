...
import cn.sunline.tag.vo.easyui.ColumnValue;
import cn.sunline.tag.vo.easyui.DataGridColumn;
import cn.sunline.tag.vo.easyui.DataGridUrl;
import cn.sunline.tag.vo.easyui.OptTypeDirection;
import cn.sunline.web.system.pojo.base.TSOperation;
...
public class DataGridTag extends TagSupport {
	...
	private boolean nowrap = true ; //是否多行显示
	
	private boolean editCell = false ; //是都单元格编辑
	...
	public void setColumn(String title, String field, Integer width,Integer showLen,String rowspan, 
			...
			boolean datebox,boolean datetimebox) {
		...
		dataGridColumn.setDatetimebox(datetimebox);
...
//					out.clearBuffer();
	...
	public StringBuffer end() {
		...
		if(editCell){//是否单元格编辑
			sb.append("$.extend($.fn.datagrid.methods, {editCell: function(jq,param){return jq.each(function(){var opts = $(this).datagrid(\'options\');" +
					"var fields = $(this).datagrid(\'getColumnFields\',true).concat($(this).datagrid(\'getColumnFields\'));	" +
					"for(var i=0; i<fields.length; i++){var col = $(this).datagrid(\'getColumnOption\', fields[i]);	col.editor1 = col.editor;if (fields[i] != param.field){" +
					"col.editor = null;	}}$(this).datagrid(\'beginEdit\', param.index);	" +
					"for(var i=0; i<fields.length; i++){var col = $(this).datagrid(\'getColumnOption\', fields[i]);	col.editor = col.editor1;}});}});");			
		}
		...
		if(this.editCell){///$(\'#" + name + "\')
			sb.append("onClickCell: function (index, field) {if (endEditing()){$(\'#" + name + "\').datagrid(\'selectRow\', index)" +
					".datagrid(\'editCell\', {index:index,field:field}); editIndex = index;}} ,");			
		}
		...
		if(editCell){
			sb.append("var editIndex = undefined;");
			sb.append("function endEditing(){if (editIndex == undefined){return true}	if ($(\'#" + name + "\').datagrid(\'validateRow\', editIndex)){$(\'#" + name + "\').datagrid(\'endEdit\', editIndex);	editIndex = undefined;	return true;} else {return false;}}");			
		}

		  
		  //		  sb.append("function onClickCell(index, field){			if (endEditing()){				$(\'#msageList\').datagrid(\'selectRow\', index)						.datagrid(\'editCell\', {index:index,field:field});				editIndex = index;			}		}");
//		sb.append("onClickCell: function (rowIndex, field) {if (endEditing()){{$(\'#msageList\').datagrid(\'selectRow\', index)" +
//		".datagrid(\'editCell\', {index:index,field:field});editIndex = index;}} ,");
//		
					...
					if (col.isQuery()) {
						...
						if("single".equals(col.getQueryMode())){
							if(!StringUtil.isEmpty(col.getReplace())){
								...
								if(col.getDefaultVal()!=null&&!col.getDefaultVal().trim().equals("")){
									if(col.isDatebox()){
										sb.append("<input onkeypress=\"EnterPress(event)\" onkeydown=\"EnterPress()\"  type=\"text\" name=\""+col.getField().replaceAll("_","\\.")+"\"  "+extendAttribute(col.getExtend())+"  class=\"easyui-datebox\" style=\"width: 100px\" value=\""+col.getDefaultVal()+"\" />");

									}else if(col.isDatetimebox()){
										sb.append("<input onkeypress=\"EnterPress(event)\" onkeydown=\"EnterPress()\"  type=\"text\" name=\""+col.getField().replaceAll("_","\\.")+"\"  "+extendAttribute(col.getExtend())+"  class=\"easyui-datetimebox\" style=\"width: 100px\" value=\""+col.getDefaultVal()+"\" />");
									}
									else{
									...
									if(col.isDatebox()){
										sb.append("<input onkeypress=\"EnterPress(event)\" onkeydown=\"EnterPress()\"  type=\"text\" name=\""+col.getField().replaceAll("_","\\.")+"\"  "+extendAttribute(col.getExtend())+"  class=\"easyui-datebox\" style=\"width: 100px\" />");

									}else if(col.isDatetimebox()){
										sb.append("<input onkeypress=\"EnterPress(event)\" onkeydown=\"EnterPress()\"  type=\"text\" name=\""+col.getField().replaceAll("_","\\.")+"\"  "+extendAttribute(col.getExtend())+"  class=\"easyui-datetimebox\" style=\"width: 100px\" />");
									}
									else{		
	...
	protected void getField(StringBuffer sb,int frozen) {
		...
		for (DataGridColumn column : columnList) {
			...
			sb.append("{field:\'" + field + "\',title:\'" + setTitleSelect(field,column.getTitle()) + "\'");
			...
			///设置 单元格标记 column ,短信模板维护页面
			if(editCell && column.getField().endsWith("Flag")){
				sb.append(",editor:{type:\'combobox\',options:{required:\'true\',panelHeight:40,valueField:\'id\',textField:\'text\',data:[{\'id\':\'1\',\'text\':\'是\'},{\'id\':\'0\',\'text\':\'否\'}]}}");
			}
	...
	public String getNoAuthOperButton(){
		...
		//cn.sunline.core.util.LogUtil.info("----getNoAuthOperButton-------"+sb.toString());
		return sb.toString(); 
	}
...
//----author:jg_longjb----start-----date:20150427--------for:新增封装查询器组件----
	...
	public String setTitleSelect(String field ,String title ){
	//	ID<BR/><select onchange=test(this)><option value = 0 >请选择</option> <option value = 1 >Volvo</option> <option value = 11>AUIDI</option> </select>
		if(editCell && field != null && field.endsWith("Flag")){
			StringBuffer sb =new StringBuffer(title);
			sb.append("<BR/>");
			sb.append("<select id = ");
			sb.append(field);
			sb.append(" onchange=allDealRules(this) style=\"WIDTH: 70px;HEIGHT:18px\"><option value =>"); 
			sb.append("请选择");
			sb.append("</option><option value = 1 >");
			sb.append("是");
			sb.append("</option> <option value = 0>");
			sb.append("否");
