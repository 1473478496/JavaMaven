...
//            var orgIds = $("#orgSelect").combobox("getValues");
		...
		function openDepartList() {
			$.dialog.setting.zIndex = getzIndex(); 
			
			var orgIds = $("#orgIds").val();
			$.dialog({content: 'url:departController.do?departList&orgIds='+orgIds, zIndex: 2100, title: '组织机构列表', lock: true, width: '800px', height: '550px', opacity: 0.4, button: [
			   {name: '<t:mutiLang langKey="common.confirm"/>',callback: callbackDepartList, focus: true},
			   {name: '<t:mutiLang langKey="common.cancel"/>', callback: function (){}}
		   ]}).zindex();
		}
		function callbackDepartList() {
			  var iframe = this.iframe.contentWindow;
...
<t:formvalid formid="formobj" dialog="true" usePlugin="password" layout="table" action="userController.do?saveUser" beforeSubmit="setOrgIds">
	...
	<table style="width: 600px;" cellpadding="0" cellspacing="1" class="formtable">
		<tr>
			...
			<td class="value" width="85%">
                ...
                <c:if test="${user.id==null }">
                    <input id="userName" class="inputxt" name="userName" validType="t_s_base_user,userName,id" value="${user.userName }" datatype="fz" />
		...
		<tr>
			...
			<td class="value">                
                <input id="departname" name="departname" type="text" readonly="readonly" class="inputxt" datatype="*" value="${tsDepart.departname}">
                <input id="departid" name="departid" type="hidden" value="${tsDepart.id}">
                <a href="#" class="easyui-linkbutton" plain="true" icon="icon-search" id="departSearch" onclick="openDepartList()">选择</a>
		...
		<tr>
			...
			<td class="value">
                <input class="inputxt" name="email" value="${user.email}" datatype="e" ignore="ignore">
                <span class="Validform_checktip"></span>
            </td>
		</tr>
	</table>
