package cn.sunline.core.extend.hqlsearch.parse;

import java.util.Arrays;

import cn.sunline.core.extend.hqlsearch.parse.vo.HqlParseEnum;
import cn.sunline.core.extend.hqlsearch.parse.vo.HqlRuleEnum;
import cn.sunline.core.util.PropertiesUtil;
...
public class PageValueConvertRuleEnum {
	...
	public static HqlRuleEnum convert(Object value) {
		...
		HqlRuleEnum rule=null;
		//判断val是否有中文字符
		boolean bol=false;
		for (int i = 0; i < val.length(); i++) {
			String strVal = val.substring(i, i + 1);  
			bol=java.util.regex.Pattern.matches("[\u4E00-\u9FA5]", strVal); 
			break;
		}
		if(bol){
			...
			// step 1 .> <
			 rule= HqlRuleEnum.getByValue(val.substring(0, 1));
			// step 2 .>= =<
