...
import cn.sunline.core.common.model.common.UploadFile;
import cn.sunline.core.common.model.json.AjaxJson;
import cn.sunline.core.common.model.json.ComboBox;
import cn.sunline.core.common.model.json.DataGrid;
import cn.sunline.core.common.model.json.ValidForm;
import cn.sunline.core.constant.Globals;
import cn.sunline.core.enums.SysThemesEnum;
import cn.sunline.core.util.ExceptionUtil;
import cn.sunline.core.util.ListtoMenu;
import cn.sunline.core.util.MyBeanUtils;
import cn.sunline.core.util.PasswordUtil;
import cn.sunline.core.util.ResourceUtil;
import cn.sunline.core.util.RoletoJson;
import cn.sunline.core.util.SetListSort;
...
import cn.sunline.tag.vo.datatable.DataTableReturn;
import cn.sunline.tag.vo.datatable.DataTables;
...
public class UserController extends BaseController {
	...
	public void menu(HttpServletRequest request, HttpServletResponse response) {
		...
		String hql="from TSUserOrg where tsUser.id='"+u.getId()+"' and tsDepart.id='"+departId+"'";
		List<TSUserOrg> uorList=systemService.findByQueryString(hql);
	...
	public List<ComboBox> role(HttpServletResponse response, HttpServletRequest request, ComboBox comboBox) {
		...
		if (StringUtil.isNotEmpty(id)) {
			...
			String hql="from TSUserOrg where tsUser.id='"+id+"' and tsDepart.id='"+departId+"'";
			List<TSUserOrg> uorList=systemService.findByQueryString(hql);
			if (uorList.size() > 0) {
				for (TSUserOrg ru : uorList) {
...
//			}
	...
	public void datagrid(TSUser user,HttpServletRequest request, HttpServletResponse response, DataGrid dataGrid) {
        ...
        sb.append(" from TSDepart where id in (");
        sb.append(" select departid from TSUser where status in(");
        sb.append("'1','-1','0'").append(")");
        sb.append(" and deleteFlag='").append(Globals.Delete_Normal).append("'");
        sb.append(" and departid like '").append(departId).append("'");
        sb.append(")");
        List<TSDepart> departList=systemService.findByQueryString(sb.toString());
        List<TSUser> cfeList = new ArrayList<TSUser>();
        List<String> orgIds=new ArrayList<String>();
        for (Object o : dataGrid.getResults()) {
        	TSUser tuser=(TSUser)o;
        	orgIds.add(tuser.getDepartid());
        	for (int i = 0; i < departList.size(); i++) {
        		TSDepart depart = departList.get(i);
        		if(depart.getId().equals(tuser.getDepartid())||depart.getId()==tuser.getDepartid()){
        			tuser.setDepartName(depart.getDepartname());
        		}
        	}
        	cfeList.add(tuser);
        }
...
//            users.setTSDepart(user.getTSDepart());
			...
			user.setUserKey(req.getParameter("roleName"));
...
//	}
	...
	public void datagridRole(TSRole tsRole, HttpServletRequest request, HttpServletResponse response, DataGrid dataGrid) {
		...
		//查询条件组装器
		cn.sunline.core.extend.hqlsearch.HqlGenerateUtil.installHql(cq, tsRole);
		this.systemService.getDataGridReturn(cq, true);
		TagUtil.datagrid(response, dataGrid);
	}
	...
	public ModelAndView addorupdate(TSUser user, HttpServletRequest req) {
		...
		if (StringUtil.isNotEmpty(user.getId())) {
			...
			req.setAttribute("user", user);
		//	idandname(req, user);
			tsDepart=systemService.get(TSDepart.class, user.getDepartid());
	...
	public ModelAndView userOrgSelect(HttpServletRequest request) {
		...
		String hql="select a from TSDepart a,TSUserOrg b where a.id=b.tsDepart.id and b.tsUser.id=?";
		List<TSDepart> orgArrList=systemService.findHql(hql, new String[]{userId});
        ...
        for (TSDepart departs : orgArrList) {
           // orgList.add((TSDepart) departs[0]);
        	 if(!orgList.contains(departs)){  
        		 orgList.add(departs);  
             }  
        }
...
//			}
	...
	public String exportXls(TSUser tsUser,HttpServletRequest request,HttpServletResponse response
		...
		CriteriaQuery cq = new CriteriaQuery(TSUser.class, dataGrid);
		cn.sunline.core.extend.hqlsearch.HqlGenerateUtil.installHql(cq, tsUser, request.getParameterMap());
		...
		for(int i=0;i<tsUsers.size();i++){
			...
			List<TSRole> roles = systemService.getSession().createSQLQuery("select * from t_s_role where id in (select roleid from t_s_user_org where userid=:userid)")
