package cn.sunline.core.interceptors;
...
import cn.sunline.core.extend.hqlsearch.SysContextSqlConvert;
import cn.sunline.core.util.ContextHolderUtils;
import cn.sunline.core.util.JeecgDataAutorUtils;
...
import cn.sunline.web.system.pojo.base.TSDataRule;
...
public class AuthInterceptor implements HandlerInterceptor {
	...
	public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object object) throws Exception {
		...
		String departId=(String) session1.getAttribute("orgId");
		...
		if (excludeUrls.contains(requestPath)) {
			...
			if (client != null && client.getUser()!=null ) {
				...
				if(!oConvertUtils.isEmpty(functionId)){
					...
					Set<String> operationCodes = systemService.getOperationCodesByUserIdAndFunctionId(departId,client.getUser().getId(), functionId);
				...
				if(!oConvertUtils.isEmpty(functionId)){
					...
					if(allOperation.size()>0){
						...
						String hasOperSql="SELECT operation FROM t_s_role_function fun, t_s_user_org role WHERE  " +
							"fun.functionid='"+functionId+"' AND fun.operation is not null  AND fun.roleid=role.role_id AND role.user_id='"+client.getUser().getId()+"' ";
					...
					Set<String> dataruleCodes = systemService.getOperationCodesByUserIdAndDataId(departId,client.getUser().getId(), functionId);
...
//			functionList = systemService.loadAll(TSFunction.class);
		...
		String sql = "SELECT DISTINCT f.id FROM t_s_function f,t_s_role_function  rf,t_s_user_org ru " +
					" WHERE f.id=rf.functionid AND rf.roleid=ru.role_id AND " +
					"ru.user_id='"+userid+"' AND f.functionurl like '"+requestPath+"%'";
		List list = this.systemService.findListbySql(sql);
		if(list.size()==0){

            String orgId = currLoginUser.getCurrentDepart().getId();

            String functionOfOrgSql = "SELECT DISTINCT f.id from t_s_function f, t_s_role_function rf, t_s_user_org ro  " +
