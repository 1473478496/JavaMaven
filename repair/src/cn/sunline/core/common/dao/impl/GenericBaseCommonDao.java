...
import cn.sunline.core.annotation.JeecgEntityTitle;
...
import cn.sunline.core.common.hibernate.qbc.DetachedCriteriaUtil;
import cn.sunline.core.common.hibernate.qbc.HqlQuery;
import cn.sunline.core.common.hibernate.qbc.PageList;
import cn.sunline.core.common.hibernate.qbc.PagerUtil;
import cn.sunline.core.common.model.common.DBTable;
import cn.sunline.core.common.model.json.DataGridReturn;
import cn.sunline.core.util.MyBeanUtils;
import cn.sunline.core.util.StringUtil;
import cn.sunline.core.util.ToEntityUtil;
...
public abstract class GenericBaseCommonDao<T, PK extends Serializable>
	...
	/**
	 * 初始化Log4j的一个实例cn.sunline.core.common.exception
	 */
	...
	private <T> void getProperty(Class entityName) {
		...
		for (int i = 0; i < str.length; i++) {
			...
			cn.sunline.core.util.LogUtil.info(property + "---&gt;" + type);
		}
	}
...
//		}
	 ...
	 * @return
	 */
	public List<T> findByQueryString(final String query,int page,int rows) {
	...
	public List<T> findListbySql(final String sql, Object[] values) {
		Query querys = getSession().createSQLQuery(sql);
		if(values != null ){
			for (int i = 0; i < values.length; i++) {
				querys.setParameter(i, values[i]);
			}			
		}
		return querys.list();
	}
	...
	public List<T> executeQueryMaxSize(final String hql, final Object[] values) {
		Query query = getSession().createQuery(hql);
