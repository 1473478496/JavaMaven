...
import com.vivebest.mall.core.entity.OrderEntity;
	...
	private void addRestrictions(CriteriaQuery<T> criteriaQuery, Pageable pageable) {
		...
		if (StringUtils.isNotEmpty(pageable.getSearchProperty()) && StringUtils.isNotEmpty(pageable.getSearchValue())) {
			...
			if(searchProperty.contains("_")){
				...
				restrictions = criteriaBuilder.and(restrictions, criteriaBuilder.like(root.get(searchPropertys[0]).<String>get(searchPropertys[1]), "%" + pageable.getSearchValue() + "%"));
			}else{
