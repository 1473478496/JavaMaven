...
import com.vivebest.mall.core.common.Template;
...
public class TemplateServiceImpl implements TemplateService, ServletContextAware {
	...
	public List<Template> getAll() {
		try {
			...
			List<Element> elements = document.selectNodes("/gbmshop/template");
	...
	public List<Template> getList(Type type) {
		if (type != null) {
			try {
				...
				Document document = new SAXReader().read(shopxxXmlFile);
				List<Template> templates = new ArrayList<Template>();
				List<Element> elements = document.selectNodes("/gbmshop/template[@type='" + type + "']");
				for (Element element : elements) {
					Template template = getTemplate(element);
					templates.add(template);
				}
	...
	public Template get(String id) {
		try {
			File shopxxXmlFile = new ClassPathResource(CommonAttributes.SHOPXX_XML_PATH).getFile();
			Document document = new SAXReader().read(shopxxXmlFile);
			Element element = (Element) document.selectSingleNode("/gbmshop/template[@id='" + id + "']");
			Template template = getTemplate(element);
			return template;
		} catch (Exception e) {
			e.printStackTrace();
