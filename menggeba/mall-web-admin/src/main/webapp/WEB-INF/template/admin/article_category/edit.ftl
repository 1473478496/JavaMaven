...
<title>${message("admin.articleCategory.edit")} - Powered By www.mgb.cn</title>
...
<body>
	...
	<form id="inputForm" action="update.jhtml" method="post">
		...
		<table class="input">
			...
			<tr>
				<th>
					${message("ArticleCategory.navigation")}:
				</th>
				<td>
					<select name="navigationId">
						<option value="">${message("admin.articleCategory.navigation")}</option>
						[#list navigations as navigation]
							[#if navigation.position == "bottom"]
								<option value="${navigation.id}" [#if navigation.id == articleCategory.navigationId] selected="selected"[/#if]>
									${navigation.name}
								</option>
							[/#if]
						[/#list]
