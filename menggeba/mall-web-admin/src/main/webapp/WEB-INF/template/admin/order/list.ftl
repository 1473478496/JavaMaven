...
<body id="bodyId">
	<form id="exportForm" method="post">
	    <input type="hidden" id="exportbeDate" name="exportbDate" value="[#if beginDate??]${(beginDate?string('yyyy-MM-dd'))!}[/#if]" />
		<input type="hidden" id="exportenDate" name="exporteDate" value="[#if endDate??]${(endDate?string('yyyy-MM-dd'))!}[/#if]" />
	</form>
	<div class="path">
	...
	<form id="listForm" action="list.jhtml" method="get">
		...
		<div class="bar">
			...
			${message("admin.purchaseRanking.beginDate")}: <input type="text" id="beginDate" name="bDate" class="text Wdate" value="[#if beginDate??]${(beginDate?string('yyyy-MM-dd'))!}[/#if]" onfocus="WdatePicker({maxDate: '#F{$dp.$D(\'endDate\')}'});" />
			${message("admin.purchaseRanking.endDate")}: <input type="text" id="endDate" name="eDate" class="text Wdate" value="[#if endDate??]${(endDate?string('yyyy-MM-dd'))!}[/#if]" onfocus="WdatePicker({minDate: '#F{$dp.$D(\'beginDate\')}'});" />
		...
		<table>
			<tr>
				...
				<td width="10%"><input type="text" name="memberName"></td>
				<td width="5%" align="right">手机号：</td>
				<td width="10%"><input type="text" name="phone"></td>
				<td width="5%" align="right">收货人：</td>
				<td width="10%"><input type="text" name="consignee"></td>
			...
			<tr>
				...
				<td width="5%" align="right">商品编号：</td>
				<td width="10%"><input type="text" name="psn"></td>
				<td width="5%" align="right">物流单号：</td>
				<td width="10%"><input type="text" name="shippingSn"></td>
				<td width="10%">
		...
		<table id="listTable" class="list">
			...
			[#list page.content as order]
				<tr>
					...
					<td>
					    [#if order.orderType="changeOrder"]${currency(order.payPrice, true)}[#else]${currency(order.amountPayable, true)}[/#if]
						[#if order.payPricePoint??]+${order.payPricePoint}萌值[/#if]
					</td>
					<td>						
						[#if order.orderType == "changeOrder"]
							是
						[#else]
							否
						[/#if]		
