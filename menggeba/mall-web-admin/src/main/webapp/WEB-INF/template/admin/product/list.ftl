...
$().ready(function() {
	...
	$exportButton.click( function() {
		var url = 'exportData.jhtml?productCategoryId='+$('#productCategoryId').val()+'&brandId='+$('#brandId').val()+'&promotionId='+$('#promotionId').val()+'&tagId='+$('#tagId').val()+'&isMarketable='+$('#isMarketable').val()+'&isList='+$('#isList').val()+'&isTop='+$('#isTop').val()
		+'&isGift='+$('#isGift').val()+'&isOutOfStock='+$('#isOutOfStock').val()+'&isStockAlert='+$('#isStockAlert').val();
...
<body>
	...
	<form id="listForm" action="list.jhtml" method="get">
		...
		<input type="hidden" id="isNewFirst" name="isNewFirst" value="[#if isNewFirst??]${isNewFirst?string("true", "false")}[/#if]" />
		<div class="bar">
			<a href="javascript:;" id="exportButton" class="button">
					${message("admin.common.export")}
			</a>
			<a href="add.jhtml?suit=false" class="iconButton">
				...
				<div class="menuWrap">
					...
					<div class="popupMenu">
						<ul id="filterOption" class="check">
							...
							<li class="separator">
								<a href="javascript:;" name="isNewFirst" val="true"[#if isNewFirst?? && isNewFirst] class="checked"[/#if]>新品首发</a>
							</li>
							<li>
								<a href="javascript:;" name="isNewFirst" val="false"[#if isNewFirst?? && !isNewFirst] class="checked"[/#if]>非新品首发</a>
							</li>
						</ul>
					</div>
				</div>
		...
		<table id="listTable" class="list">
			<tr>
				...
				<th>
					<a href="javascript:;" class="sort" name="pricePoint">${message("Product.price.point")}</a>
				</th>
				<th>
				...
				<tr>
					...
					<td>
						<span title="${product.fullName}">
							<a href="${toView}${product.path}" target="_blank">${abbreviate(product.fullName, 50, "...")}</a>
							[#if product.isGift]
					...
					<!-- 显示所需积分 modify by huangjinhua 20150724-->
					<td>
						${currency(product.pricePoint)}
					</td>
