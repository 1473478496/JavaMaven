...
<title>${message("admin.productNotify.list")} - Powered By www.mgb.cn</title>
	...
	<form id="listForm" action="list.jhtml" method="get">
		...
		<table id="listTable" class="list">
			<tr>
				...
				<th>
					<a href="javascript:;" class="sort" name="member">${message("Member.mobile")}</a>
				</th>
				<th>
			...
			[#list page.content as productNotify]
				<tr>
					<td>
						<input type="checkbox" [#if !productNotify.product.isMarketable || productNotify.product.isOutOfStock]disabled="disabled"[/#if] name="ids" value="${productNotify.id}" />
					</td>
					<td>
						<a href="${toView}${productNotify.product.path}" title="${productNotify.product.fullName}" target="_blank">${abbreviate(productNotify.product.fullName, 50, "...")}</a>
					</td>
					<td>
