...
<title>${message("admin.secKill.edit")} - Powered By www.mgb.cn</title>
...
<body>
	<div class="path">
		<a href="${base}/admin/common/index.jhtml">${message("admin.path.index")}</a> &raquo; ${message("admin.secKill.edit")}
	</div>
	<form id="inputForm" action="update.jhtml" method="post">
		...
		<div class="tabContent">
			...
			<table class="input">
				<tr>
				   <th><span class="requiredField">*</span>秒杀时间</th>
				   <td>
				  	    <select id="beginTime" name="promotionId" value="selected">
							<option value="${special.promotion.id}">${special.promotion.beginDate?string("yyyy-MM-dd HH:mm:ss")}</option>
							 [#list promotions as promotion]
         					 	<option value="${promotion.id}">${promotion.beginDate?string("yyyy-MM-dd HH:mm:ss")}</option>
							 [/#list]
      					 </select>
				   </td>
				</tr>
				...
				<tr>
					...
					<td colspan="2">
						<label>
							<input type="checkbox" id="isStick" name="isStick" value="true" [#if special.isStick == true] checked="checked"[/#if]/>${message("admin.secKill.stick") }
						</label>
					</td>
				</tr>
				<tr id="stickImage" [#if special.isStick == false] class="hidden"[/#if]>
					...
					<td colspan="2">
						<span class="fieldSet">
							<input type="text" name="stickImage" class="text" value="${special.stickImage }" [#if special.isStick == false] disabled="disabled"[/#if] maxlength="200" title="${message("admin.secKill.imageTitle")}" />
