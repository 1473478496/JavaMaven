...
$().ready(function() {
	...
	*/
	...
	[#if !order.expired && order.orderStatus == "paymented" && (order.shippingStatus == "unshipped" || order.shippingStatus == "partialShipment")]
		// 发货
	...
	<table class="input tabContent">
		<tr>
			...
			<td>
				...
				<input type="button" id="shippingButton" class="button" value="${message("admin.order.shipping")}"[#if order.expired || order.orderStatus != "paymented" || (order.shippingStatus != "paid" && order.shippingStatus != "unshipped" )||(order.quantity-order.shippedQuantity-order.returnQuantity)<1] disabled="disabled"[/#if] />
				<input type="button" id="completeButton" class="button" value="${message("admin.order.complete")}"[#if order.expired || order.orderStatus != "shipped"] disabled="disabled"[/#if] />
			</td>
			<td>
				&nbsp;
			</td>
