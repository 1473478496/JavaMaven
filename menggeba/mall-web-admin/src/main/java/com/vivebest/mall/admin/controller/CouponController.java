...
import com.vivebest.mall.core.common.ExcelView;
...
public class CouponController extends BaseController {
	...
	public String save(Coupon coupon, Long[] productCategoryIds, Long[] brandIds,  Long[] productIds,  RedirectAttributes redirectAttributes) {
		if (!isValid(coupon)) {
			return ERROR_VIEW;
		}
		...
		//coupon.setBrands(new HashSet<Brand>(brandService.findList(brandIds)));
		for (Product product : productService.findList(productIds)) {
			if (!product.getIsGift()) {
				coupon.getProducts().add(product);
			}
		}
		if(coupon.getProductCategories().size()==0&&coupon.getProducts().size()==0) //都为空，即为全场通用
		{
			coupon.setIsFullfield(true);
		}		 
		 else
		{
			coupon.setIsFullfield(false);
		}
	...
	public String update(Coupon coupon, Long[] productCategoryIds, Long[] brandIds,  Long[] productIds, RedirectAttributes redirectAttributes) {
		if (!isValid(coupon)) {
			return ERROR_VIEW;
		}
		if (coupon.getBeginDate() != null && coupon.getEndDate() != null && coupon.getBeginDate().after(coupon.getEndDate())) {
