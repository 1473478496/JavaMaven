...
$(document).ready(function(){
    $("#alter").blur(function(){
        var vs=$("#alter").val();
          if(vs){
            if(/^[0-9]*$/.test(vs)){
               $("#alertAmountPaid").html("订单总金额<em>"+vs+"</em>元");
             }else{
               alert("您的输入有误!");
               $("#alter").val("");
             }
  ...
  <div class="wrap">
    <div class="container-setting fix">
     ...
     <!-- 菜单 -->
      ...
      <div class="container-middle">
          ...
          <div>
            <div class="order-control">
             <form action="${base}/orderItem/alter.do" method="get">
				...
				<div class="settle_control">
	              <div class="order-list settlescroll">
	               ...
	               <div class="slcomfir_info">
	                  	<!-- 结算总金额 -->
	              		<p class="settletit"><span id="alertAmountPaid">订单总金额<em>${order.amountPaid}</em>元</span></p>
	              		<p class="settletit"><span>实付   ${order.offsetAmount}  元</span></p>
	              		
	              		<!-- 如果是点击修改价格按钮,出现-金额变成可修改  -->
	              		<p class="settletit"><span>订单总金额<input id="alter" type="text" class="order_input" value=""/>元</span></p>
	              		
	              		<!--点击修改出现   按钮 取消按钮返回之前页面 -->
		              	<div class="orderbtnbox">
		                  <input type="button" value="确认修改" onclick="doAlter()">
	  ...
	  <!-- 提示弹框 -->
	   <div class="tip_box">
	    <div class="tip_title">修改成功</div>
	    <div class="tip_main">正在跳转到订单列表...</div>
	
	    <div class="tip_footer">
	      <a class="btn btn_yes">点击跳转...</a>
	    </div>
	  </div>
	  <div class="tip_bg"></div>
	   <script>
		  ...
		  function tip_fadeIn(){
		      $('.tip_bg').fadeIn();
		      $('.tip_box').show().animate({"opacity":"1"},200);
		  }
		  // 关闭弹框
		  function tip_fadeOut(){
		      $('.tip_box').animate({"opacity":"0"},200,function(){$('.tip_box').hide()});
		      $('.tip_bg').fadeOut();
		  }
