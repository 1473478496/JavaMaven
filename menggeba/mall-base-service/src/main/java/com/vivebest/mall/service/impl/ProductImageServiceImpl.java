...
import com.vivebest.mall.core.util.ImageUtils;
...
public class ProductImageServiceImpl implements ProductImageService,
	 ...
	 *            原文件类型
	 */
	private void addTask(final String sourcePath, final String largePath,
				...
				public void run() {
					...
					for (StoragePlugin storagePlugin : storagePlugins) {
						if (storagePlugin.getIsEnabled()) {
							...
							File mediumTempFile = new File(tempPath
							...
							try {
								...
								ImageUtils.addWatermark(largeTempFile,
										largeTempFile, watermarkFile,
										setting.getWatermarkPosition(),
										setting.getWatermarkAlpha());
								ImageUtils.zoom(tempFile, mediumTempFile,
										setting.getMediumProductImageWidth(),
										setting.getMediumProductImageHeight());
								ImageUtils.addWatermark(mediumTempFile,
								...
								ImageUtils
										.zoom(tempFile,
												thumbnailTempFile,
												setting.getThumbnailProductImageWidth(),
												setting.getThumbnailProductImageHeight());
								storagePlugin.upload(sourcePath, tempFile,
										contentType);
								storagePlugin.upload(largePath, largeTempFile,
										DEST_CONTENT_TYPE);
								storagePlugin.upload(mediumPath,
										mediumTempFile, DEST_CONTENT_TYPE);
								storagePlugin.upload(thumbnailPath,
										thumbnailTempFile, DEST_CONTENT_TYPE);
							} finally {
			...
			try {
				...
				for (StoragePlugin storagePlugin : storagePlugins) {
					if (storagePlugin.getIsEnabled()) {
						...
						addTask(sourcePath, largePath, mediumPath,
								thumbnailPath, tempFile,
								multipartFile.getContentType());
						productImage
								.setSource(storagePlugin.getUrl(sourcePath));
						productImage.setLarge(storagePlugin.getUrl(largePath));
						productImage
								.setMedium(storagePlugin.getUrl(mediumPath));
						productImage.setThumbnail(storagePlugin
								.getUrl(thumbnailPath));
					}
				}
			} catch (Exception e) {
				e.printStackTrace();
	...
	public List<ProductImage> buildSource(String productImgSour) {
		List<ProductImage> produtimglist = null;
		if (productImgSour != null && !productImgSour.isEmpty()) {
			try {
				String[] splitSource = productImgSour.split(";");
				
				if (splitSource != null && splitSource.length > 1) {
					Setting setting = SettingUtils.get();
					Map<String, Object> model = new HashMap<String, Object>();
					String uploadPath = FreemarkerUtils.process(
							setting.getImageUploadPath(), model);
					...
					produtimglist = new ArrayList<ProductImage>();
					ProductImage productImage = new ProductImage();
					
					productImage
							.setLarge(!splitSource[0].isEmpty() ? storagePlugin.getUrl(uploadPath+splitSource[0]
									.toString()) : null);
					productImage
							.setMedium(!splitSource[1].isEmpty() ? storagePlugin.getUrl(uploadPath+splitSource[1]
									.toString()) : null);
					
					productImage
					.setSource(!splitSource[1].isEmpty() ? storagePlugin.getUrl(uploadPath+splitSource[1]
							.toString()) : null);
					
					productImage
							.setThumbnail(!splitSource[2].isEmpty() ? storagePlugin.getUrl(uploadPath+splitSource[2]
									.toString()) : null);
					produtimglist.add(productImage);
						}
					}
				}
