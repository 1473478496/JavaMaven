...
public class ArticleCategoryDaoImpl extends BaseDaoImpl<ArticleCategory, Long> implements ArticleCategoryDao {
	 ...
	 * @param count
	 * @return 
	 * @see com.vivebest.mall.dao.ArticleCategoryDao#findRoots(com.vivebest.mall.entity.Navigation, java.lang.Integer) 
	 */
	@Override
	public List<ArticleCategory> findRoots(Navigation navigation, Integer count) {
		String jpql = "select articleCategory from ArticleCategory articleCategory where articleCategory.navigationId=" + navigation.getId() + "order by articleCategory.order asc";
		TypedQuery<ArticleCategory> query = entityManager.createQuery(jpql, ArticleCategory.class).setFlushMode(FlushModeType.COMMIT);
		if (count != null) {
			query.setMaxResults(count);
		}
