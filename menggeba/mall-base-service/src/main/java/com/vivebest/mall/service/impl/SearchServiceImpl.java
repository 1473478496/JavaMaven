...
//        reader.close();  
	...
	@SuppressWarnings("unchecked")
	@Transactional(readOnly = true)
	public Page<Product> search(String keyword, BigDecimal startPrice, BigDecimal endPrice, OrderType orderType, Pageable pageable, Boolean isStore) {
...
//			QueryParser nameParser = new QueryParser(Version.LUCENE_35, "name", new StandardAnalyzer(Version.LUCENE_35));
//			nameParser.setDefaultOperator(QueryParser.OR_OPERATOR);
...
//			Query nameFuzzyQuery = new WildcardQuery(new Term("name", "*"+text+"*"));
//			QueryParser fullNameParser = new QueryParser(Version.LUCENE_35, "fullname", new StandardAnalyzer(Version.LUCENE_35));
//			fullNameParser.setDefaultOperator(QueryParser.OR_OPERATOR);
//			Query fullNameQuery = fullNameParser.parse(text);
//			Query fullNameFuzzyQuery = new WildcardQuery(new Term("fullname", "*"+text+"*"));
...
//			TermQuery suitQuery = new TermQuery(new Term("suit", "false"));
//			BooleanQuery textQuery = new BooleanQuery();
//			BooleanQuery query = new BooleanQuery();
...
//			textQuery.add(fullNameQuery, Occur.SHOULD);
//			textQuery.add(fullNameFuzzyQuery, Occur.SHOULD);
//			query.add(isMarketableQuery, Occur.MUST);
//			query.add(isListQuery, Occur.MUST);
//			query.add(isGiftQuery, Occur.MUST);
//			query.add(suitQuery, Occur.MUST);
//			query.add(textQuery, Occur.MUST);
//			if (isStore != null && isStore) {
//				NumericRangeQuery<Integer> numericRangeQuery = NumericRangeQuery.newIntRange("stock", 0, 0, true, true);
//				query.add(numericRangeQuery, Occur.MUST_NOT);
//			}
//			
