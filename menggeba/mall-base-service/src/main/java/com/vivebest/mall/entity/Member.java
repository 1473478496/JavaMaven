...
import com.vivebest.mall.interceptor.MemberInterceptor;
 ...
 */
@Entity
@Table(name = "gbm_member")
@SequenceGenerator(name = "sequenceGenerator", sequenceName = "gbm_member_sequence")
public class Member extends BaseEntity {
	...
	/** email的安全密匙 */
	private EmailSafeKey emailSafeKey;
	...
	/** 权益订单 */
	private Set<RightOrder> rightsOrders = new HashSet<RightOrder>();
	...
	/**权益吗*/
	private Set<RightsCode> rightsCodes = new HashSet<RightsCode>();
	...
	/** 边民信息 */
	private Set<BorderMan> borderMans = new HashSet<BorderMan>();
...
//	@Field(store = Store.YES, index = Index.NO)
	 ...
	 */
	@Embedded
	public EmailSafeKey getEmailSafeKey() {
		return emailSafeKey;
	}
	 ...
	 *            安全密匙
	 */
	public void setEmailSafeKey(EmailSafeKey emailSafeKey) {
		this.emailSafeKey = emailSafeKey;
	}
	 ...
	 */
	@OneToMany(mappedBy = "member", fetch = FetchType.LAZY, cascade = CascadeType.REMOVE)
	public Set<RightOrder> getRightsOrders() {
		return rightsOrders;
	}
	 ...
	 * @param rightsOrders
	 */
	public void setRightsOrders(Set<RightOrder> rightsOrders) {
		this.rightsOrders = rightsOrders;
	}
	...
	@JoinTable(name = "gbm_member_favorite_product")
	@OrderBy("createDate desc")
	...
	public void setAttributeValue(MemberAttribute memberAttribute, Object attributeValue) {
		if (memberAttribute != null) {
			...
			} else if (memberAttribute.getType() == Type.hobby
					&& (attributeValue instanceof String || attributeValue == null)) {
				setHobby((String) attributeValue);
			}else if (memberAttribute.getType() == Type.checkbox
	...
	public Boolean getIsMegValidate() {
		return isMegValidate;
	}

	public void setIsMegValidate(Boolean isMegValidate) {
		this.isMegValidate = isMegValidate;
	}
	...
	public String getCheckEmail() {
		if (this.email.indexOf("@") > 3) {
			return this.email.substring(0, 3) + "****"
					+ this.email.substring(this.email.indexOf("@"), this.email.length());
		} else {
	...
	@OneToMany(mappedBy = "member", fetch = FetchType.LAZY, cascade = CascadeType.REMOVE)
	public Set<RightsCode> getRightsCodes() {
		return rightsCodes;
	}

	public void setRightsCodes(Set<RightsCode> rightsCodes) {
		this.rightsCodes = rightsCodes;
	}
	...
	@JoinTable(name = "gbm_member_follow_brand",
	joinColumns = {@JoinColumn(name = "members", referencedColumnName = "id")},
	...
	@OneToMany(mappedBy = "member", fetch = FetchType.LAZY, cascade = CascadeType.REMOVE)
	public Set<BorderMan> getBorderMans() {
		return borderMans;
	}

	public void setBorderMans(Set<BorderMan> borderMans) {
		this.borderMans = borderMans;
	}
