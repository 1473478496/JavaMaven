...
@Entity
@Table(name = "gbm_product")
@SequenceGenerator(name = "sequenceGenerator", sequenceName = "gbm_product_sequence")
public class Product extends BaseEntity {
	...
	/** 搭配销售 */
	private Set<TieInSale> tieInSale = new HashSet<TieInSale>();

	/** 首页猜你喜欢 */
	private Set<GuessYouLike> guessYouLikes = new HashSet<GuessYouLike>();

	private Set<ProductRecommond> productRecommonds = new HashSet<ProductRecommond>();
	...
	static {
		try {
			...
			org.dom4j.Element element = (org.dom4j.Element) document
					.selectSingleNode("/gbmshop/template[@id='productContent']");
	 ...
	 *            是否为新品首发
	 */
	public void setIsNewFirst(Boolean isNewFirst) {
	...
	@CollectionTable(name = "gbm_product_product_image")
	...
	@JoinTable(name = "gbm_product_tag")
	@OrderBy("order asc")
	 ...
	 */
	@ManyToMany(fetch = FetchType.LAZY)
	@JoinTable(name = "gbm_product_specification")
	@OrderBy("order asc")
	 ...
	 */
	@ManyToMany(fetch = FetchType.LAZY)
	@JoinTable(name = "gbm_product_specification_value")
	...
	@CollectionTable(name = "gbm_product_member_price")
	 ...
	 */
	@ElementCollection(fetch = FetchType.LAZY)
	@CollectionTable(name = "gbm_product_parameter_value")
	...
	public String getWapPath() {
		...
		model.put("productCategory", getProductCategory());
		try {
			return FreemarkerUtils.process("/wap" + staticPath, model);
		} catch (IOException e) {
			e.printStackTrace();
		} catch (TemplateException e) {
			e.printStackTrace();
	...
	public Set<TieInSale> getTieInSale() {
		return tieInSale;
	}

	public void setTieInSale(Set<TieInSale> tieInSale) {
		this.tieInSale = tieInSale;
	}
	...
	public Set<GuessYouLike> getGuessYouLikes() {
		return guessYouLikes;
	}
	 ...
	 *            首页猜你喜欢
	 */
	public void setGuessYouLikes(Set<GuessYouLike> guessYouLikes) {
		this.guessYouLikes = guessYouLikes;
	}

	@OneToMany(mappedBy = "product", fetch = FetchType.LAZY)
	public Set<ProductRecommond> getProductRecommonds() {
		return productRecommonds;
	}

	public void setProductRecommonds(Set<ProductRecommond> productRecommonds) {
